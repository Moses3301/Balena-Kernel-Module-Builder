...
build_module() {
  ...
  mkdir -p build
  cp -r src/* build
  echo "CONFIG_HID_ITHC=m" > build/.config
  make -C "${headers_dir}" modules_prepare
  make -C "${headers_dir}" M="$PWD/build" modules CONFIG_HID_ITHC=m V=1
}
...
Device types: ${slug}"
    ...
    if [ ! -d "$output_dir" ]; then
      ...
      info "Module for ${slug} at version ${os_version} already built. Skipping."
    fi
  fi
}
